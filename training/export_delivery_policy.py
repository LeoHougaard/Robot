...
def export(checkpoint, output, profile_path, fit_path, evaluations, fidelity):
    ...
    metadata = dict(
        ...
        posture_command_contract=dict(height_offset_m=[-.01, .01], roll_rad=[-.06, .06], pitch_rad=[-.06, .06],
                                      nominal_height_m=float(cfg["nominal_support_height_m"]),
                                      height_reference="root vertical clearance above mean terrain under chassis",
                                      attitude_reference="gravity", terrain_measurement_in_actor=False,
