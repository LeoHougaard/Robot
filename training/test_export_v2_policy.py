...
class DeploymentContractTests(unittest.TestCase):
    ...
    def test_accepts_measured_25_hz_control_contract(self):
        self.contract(control_hz=25)
        with self.assertRaisesRegex(ValueError, "Control rate"):
            self.contract(control_hz=30)
    ...
    def test_current_export_requires_the_complete_current_screen(self):
        ...
        validate_current_evaluation(
            {
                "stage": "current",
                "passed": True,
                "segments": {name: {} for name in EXPECTED["current"]},
            }
        )
        validate_current_evaluation(
            {
