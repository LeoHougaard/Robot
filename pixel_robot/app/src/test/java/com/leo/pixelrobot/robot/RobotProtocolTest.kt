...
class RobotProtocolTest {
    ...
    @Test
    fun capturedZeroStepFitsComfortablyInsideTheFirmwareJsonBuffer() {
        val poses = List(1) { step ->
        ...
        assertEquals(1, command.getJSONArray("steps").length())
        assertEquals(162.0, command.getJSONArray("steps").getJSONObject(0)
            .getJSONObject("poses").getDouble("12"), 1.0e-6)
        assertTrue(bytes.size < 1_024)
    }

    @Test
    fun capturedZeroStepRejectsMultipleSteps() {
        val pose = (1..12).associateWith { 180f }
        val error = runCatching {
            RobotProtocol.playPoseSequence(List(2) { pose }, 200, 180, 30)
        }.exceptionOrNull()
